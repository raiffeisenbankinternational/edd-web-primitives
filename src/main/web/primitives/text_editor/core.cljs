...
(defn- edit-mode [props set-read-only-mode]
  [:> Grid (merge
            {:container true}
            (if (:id props) {:id (str "editor-" (:id props))}))
   [:> SunEditor
    (merge
   ...
   [:> Grid {:container true :style {:border "solid 1px #dadada"
    ...
    (if (contains? props :on-save)
      [:> Grid {:item true :style {:padding "1rem"}}
       [:> Button (merge
                   {:variant  "outlined"
                    :on-click (handle-on-save props set-read-only-mode)
                    :style    {:height "2rem"}}
                   (if (:id props) {:id (str "save-button-" (:id props))})) "Save"]
       [:> Button (merge
                   {:on-click set-read-only-mode
                    :style    {:height "2rem"}}
                   (if (:id props) {:id (str "discard-button-" (:id props))})) "Discard"]])]])
